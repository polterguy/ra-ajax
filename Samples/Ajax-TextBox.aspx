...
<asp:Content 
    ...
    <p>
        ...
        TextBox sample will still work. The reason is that we trap *changes when they happen* instead of
        "diffing" the "incoming page" against the "outgoing" page. While many other Ajax Frameworks will
        actually *diff* the content after ViewState is loaded against the content sent back to the client
        when the Control(s) are about to be rendered and then send that "diff". Ra-Ajax intercepts when you
        set a value of a property and then stores that value for later sending it back to the client.
    </p>
    <p>
